...
static void *ht_realloc(void *ht, size_t new_capacity, vl_realloc_t realloc_, vl_source_location_t loc) {
    VL_HT_HEADER(ht)->capacity = new_capacity;
    if (realloc_) {
        return realloc_(VL_HT_HEADER(ht), new_capacity);
    } else {
        return vl_realloc(VL_HT_HEADER(ht), new_capacity, loc);
    }
}
...
void *vl_ht_put(void **ht, void *key, void *value, size_t key_size, size_t value_size, 
    ...
    if (header->count >= header->capacity) {
        *ht = ht_realloc(*ht, header->capacity * 2, realloc_, loc);
        header = VL_HT_HEADER(*ht);
    }
