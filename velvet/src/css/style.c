...
vl_result_t vl_css_inline_style_init_(vl_css_inline_style_t *style, const char *css, vl_source_location_t loc) {
    if (!style || !css) return VL_ERROR;
    style->rules = VL_DA_INIT(vl_css_rule_t);
    vl_css_parser_t parser = {0};
    vl_css_parser_init(&parser, css, loc);
    vl_css_rule_t tmp = {0};
    vl_result_t result = VL_SUCCESS;
    while (result != VL_STOP) {
...
vl_css_value_t vl_css_style_get_property(vl_css_style_t *style, const char *property, vl_css_value_t fallback) {
    ...
    if (style->applied_rules) {
        for (int i = 0; i < VL_DA_LENGTH(style->applied_rules); i++) {
            const vl_css_rule_t *rule = style->applied_rules[i];
...
vl_result_t vl_css_style_merge(vl_css_style_t *dst, const vl_css_style_t *style) {
    ...
    for (int i = 0; i < len; i++) {
        const vl_css_rule_t *rule = style->applied_rules[i];
        const vl_css_rule_t **duplicate_rule = NULL;
        for (int j = 0; j < VL_DA_LENGTH(dst->applied_rules); j++) {
            const vl_css_rule_t *dst_rule = dst->applied_rules[j];
        ...
        if (duplicate_rule) {
            const vl_css_rule_t *old_rule = *duplicate_rule;
...
vl_result_t vl_css_style_merge_inline(vl_css_style_t *dst, const vl_css_inline_style_t *style) {
    if (!dst || !style) return VL_ERROR;
    if (!dst->applied_rules || !style->rules) return VL_ERROR;
    int len = VL_DA_LENGTH(style->rules);
    for (int i = 0; i < len; i++) {
        const vl_css_rule_t *rule = style->rules + i;
        const vl_css_rule_t **duplicate_rule = NULL;
        for (int j = 0; j < VL_DA_LENGTH(dst->applied_rules); j++) {
            const vl_css_rule_t *dst_rule = dst->applied_rules[j];
            if (dst_rule->property && strcmp(rule->property, dst_rule->property) == 0) {
...
static void print_size_metric(const vl_css_size_metric_t *metric) {
...
static void print_value(const vl_css_value_t value) {
    switch (value.type) {
...
static void print_rule(const vl_css_rule_t *rule) {
    printf("%s: ", rule->property);
    print_value(rule->value);
    if (rule->important) {
        printf(" !important");
    }
    printf(";");
...
vl_result_t vl_css_inline_style_deinit(vl_css_inline_style_t *style) {
    if (!style) return VL_ERROR;
    if (style->rules) {
        for (int i = 0; i < VL_DA_LENGTH(style->rules); i++) {
            vl_css_rule_deinit(style->rules + i);
        }
    }
    VL_DA_FREE(style->rules);
    return VL_SUCCESS;
}
