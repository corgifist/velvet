...
vl_dom_element_t *vl_dom_element_span_new(vl_source_location_t loc) {
    vl_dom_element_funcs_t *funcs = vl_malloc(sizeof(vl_dom_element_funcs_t) + sizeof(vl_dom_element_span_t));
    funcs->render = vl_dom_element_span_render;;
    funcs->free = vl_dom_element_span_free;
    vl_dom_element_span_t *p = VL_PTR_FORWARD(funcs, sizeof(vl_dom_element_funcs_t));
    p->base.tag = "span";
