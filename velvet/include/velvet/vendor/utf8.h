...
// ICU compatibility layer
#define U8_LENGTH utf8codepointsize
#define U8_NEXT(STRING, INDEX, LEN, CHAR) \
    do { \
        size_t __len = LEN <= 0 ? strlen(STRING) : LEN; \
        if (INDEX > __len) { \
            CHAR = 0; \
            break; \
        } \
        utf8codepoint(STRING + INDEX, &CHAR); \
        INDEX += U8_LENGTH(CHAR); \
    } while (0)
